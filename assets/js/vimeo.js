const startVimeo = vimeoData => {
  ...
  const updateFirstClickBox = () => {
    if (localStorage.getItem('vimeo-video-finished')) {
      vimeoFirstClickBoxPRef.textContent = vimeoData.text.replay;
    } else {
      vimeoFirstClickBoxPRef.textContent = vimeoData.text.initial;
    }

    if (!isNaN(parseFloat(localStorage.getItem('vimeo-current-time')))) {
      vimeoFirstClickBoxPRef.textContent = vimeoData.text.resume;
    }
  };
